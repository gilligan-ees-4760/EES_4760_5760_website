...
to scout  ; a turtle procedure
  ...
  ; Test output
   file-type (word who "," month "," is-alpha? "," is-female? "," age-in-months "," I-should-scout "," elders "," p-alpha "," p-scout ",")
...
to-report p-survive
    report survival-prob ^ (12 - month)
end

to-report p-alpha
    report (1 - 0.99 ^ 12 ) ^ (elders + 1)

    ; let m month
    ; if m > 10 [set m m - 12]
    ; let p-survive survival-prob ^ (12 - m)
    ; report (1 - p-survive) ^ elders
end
...
to-report p-scout
  ...
  [
    let ratio p-vacant * scouting-survival * survival-prob / p-alpha
    set ratio ln(ratio)
    let z exp(a-log + b-log * ratio)
    report z / (1 + z)
    ;let p 1 - p-alpha / scouting-survival
    ;if p < 0 [set p 0]
    ;report p
  ]
  stop
