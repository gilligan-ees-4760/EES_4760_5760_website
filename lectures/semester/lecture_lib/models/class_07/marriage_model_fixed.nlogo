...
to marry
 ...
 ; FIX ERROR move social-network calculation to submodel and fix it there.
 set social-network social-net
  ; Test output:
    file-type count social-network      file-type ","

  ; Evaluate the married fraction of social network
  ; FIX ERROR check for division by zero
   if not any? social-network [ stop ]
 ...
 ; Test output:
   ...
   ; FIX ERROR compare to social-pressure, not married-fraction
   ...
   let potential-partners social-network with
    ...
    ask my-spouse
     [
       set is-married? true
       set color green
       ; FIX ERROR remember to set my-spouses-social-angle
       set my-spouses-social-angle [ycor] of myself
     ]
...
; FIX ERROR break social network calculation out for testing and debugging
; Social Network submodel
to-report social-net
 let ycmin ycor - social-network-angle-range
 let ycmax ycor + social-network-angle-range
 ; Handle case where one is on the top and the other is on the bottom.
 let ycmin2 ycmin
 let ycmax2 ycmax
 if ycmin < 0 [
   set ycmin2 ycmin + 360
   set ycmax2 360
   set ycmin 0
 ]
 if ycmax > 360 [
   set ycmin2 0
   set ycmax2 ycmax - 360
   set ycmax 360
 ]

; print (word "ycmin = " ycmin ", ycmax = " ycmax ", ycmin2 = " ycmin2 ", ycmax2 = " ycmax2)

 let snet other turtles with
 [
   xcor >= ([xcor] of myself - social-network-age-range) and
   xcor <= ([xcor] of myself + social-network-age-range) and
   ( (ycor >= ycmin and ycor <= ycmax) or
   (ycor >= ycmin2 and ycor <= ycmax2) )
 ]
 report snet
end
   ...
   [
     hatch 1
       [
         ...
         ; FIX ERROR set baby's social angle properly
         let angle-difference subtract-headings
          ([my-spouses-social-angle] of myself) ([ycor] of myself)
         set ycor [ycor] of myself + random-float angle-difference
         if ycor < 0 [ set ycor ycor + 360 ]
         if ycor > 360 [ set ycor ycor - 360 ]
