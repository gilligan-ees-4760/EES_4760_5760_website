...
to find-risk
  ...
  [
    setup-experiment
    while [ticks < 100 and any? turtles with [color = red]] [ go ]
    set extinctions extinctions + red-extinct
    set runs runs + 1
  ]
  set risk extinctions / runs
end
...
* Wilensky, U. (1997).  NetLogo Simple Birth Rates model.  https://ccl.northwestern.edu/netlogo/models/SimpleBirthRates.  Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
...
@#$#@#$#@
<experiments>
  <experiment name="ex_8.1" repetitions="20" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>red-count = 0</exitCondition>
    <metric>count turtles</metric>
    <steppedValueSet variable="carrying-capacity" first="500" step="500" last="4000"/>
    <enumeratedValueSet variable="red-fertility">
      <value value="2"/>
    </enumeratedValueSet>
    <steppedValueSet variable="blue-fertility" first="2.1" step="0.1" last="5"/>
  </experiment>
  <experiment name="ex_8.2" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>find-risk
stop</go>
    <metric>risk</metric>
    ...
    <steppedValueSet variable="blue-fertility" first="2" step="0.1" last="5"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
