...
turtles-own [
  strategy ;0=defector, 1=cooperator/TFT
  payoff ;;payoff
]
...
to go
  if (all? turtles [strategy = 0] or all? turtles [strategy = 1]) [stop] ;;stop if one stategy has "won"
...
to play-game
  ...
  ask turtles [
    ...
    [ set payoff num-iterations * (neighbors-C * (payoff-benefit - payoff-cost) - neighbors-D * payoff-cost) ]
    ...
    if (strategy = 0) [
      ifelse TFT?
      [ set payoff (neighbors-C * payoff-benefit) ] ;;exploiting TFT agents
      [ set payoff num-iterations * (neighbors-C * payoff-benefit) ] ;;exploiting cooperators
    ]
  ]
end
