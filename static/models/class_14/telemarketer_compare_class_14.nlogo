...
to do-accounting
  ifelse mergers?
  [ do-accounting-with-mergers]
  [ do-accounting-without-mergers]
end


to do-accounting-without-mergers
  set balance balance + (2 * successful-calls) - (50 * size)
...
to update-observers
  ...
  [
    ...
    set-plot-x-range 0 (ceiling (1.1 * max [size] of turtles))
  ]
...
<experiments>
  <experiment name="vary_initial_telemarketers" repetitions="20" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="limited-calls">
      <value value="true"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="remove-limits" repetitions="20" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="model_comparison" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="growth-param">
    ...
    <steppedValueSet variable="initial-num-marketers" first="50" step="50" last="500"/>
    <enumeratedValueSet variable="limit-calls?">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="mergers?">
      <value value="false"/>
      <value value="true"/>
    </enumeratedValueSet>
  </experiment>
