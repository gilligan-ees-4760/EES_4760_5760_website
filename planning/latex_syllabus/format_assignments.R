...
p_load(yaml)
p_load(here)
p_load_current_gh("jonathan-g/cmarkjg")
p_load(digest)
...
load_semester_db <- function(semester_db) {
  ...
  text_code_digests <- tibble(value = text_codes,
                              code = str_c("<%", names(text_codes), "%>"),
                              digest = str_c("XabYcdZ", map_chr(code, digest),
                                             "ZdcYbaX"))
  ...
  if (has_hw_assignments) {
    homework_assignments <- hw_assignments %>%
      ...
      select(homework_id, hw_group, hw_due_date,
             hw_type, med_hw_type, short_hw_type,
...
digest_text_codes <- function(txt) {
  repl <- with(text_code_digests, fixed(set_names(digest, code)))
  str_replace_all(txt, repl)
}

undigest_text_codes <- function(txt) {
  repl <- with(text_code_digests, fixed(set_names(code, digest)))
  str_replace_all(txt, repl)
}

markdown_and_expand <- function(txt) {
  if (length(txt) > 1) stop("markdown_and_expand does not accept lists or vectors")
  if (is.na(txt)) return(NA_character_)
  txt <- txt %>% digest_text_codes %>%
    markdown_latex(smart = TRUE, normalize = TRUE, extensions = TRUE) %>%
    undigest_text_codes %>% expand_codes
  pattern_1 <- "(\\\\textless\\{\\}[[:alnum:]$\\-_.,+!*()'?&/:;@=]+)\\\\([[:alnum:]$\\-_.,+!*()'?&/:;@=\\\\]+\\\\textgreater\\{\\})"
  pattern_2 <- "\\\\textless\\{\\}([[:alnum:]$\\-_.,+!*()'?&/:;@=]+)\\\\textgreater\\{\\}"
  while(str_detect(txt, pattern_1)) {
    txt <- str_replace_all(txt, pattern_1, "\\1\\2")
  }
  txt <- str_replace_all(txt, pattern_2, "\\\\url{\\1}")
  txt
}
...
format_day_date_range_by_event_id <- function(calendar, event_ids, abbr_month = TRUE, abbr_wday = TRUE) {
  events <- calendar %>% filter(event_id %in% event_ids)
  format_day_date_range(events$date, abbr_month = abbr_month, abbr_wday = abbr_wday)
}
...
make_reading_assignment <- function(reading_entry) {
  ...
  reading_notes <- reading_entry %>% filter(!is.na(reading_notes)) %>%
    mutate(reading_notes = map_chr(reading_notes, markdown_and_expand) %>%
             str_trim() )
...
make_homework_name <- function(homework_entries) {
  ...
  if (length(hw_type) == 0) hw_type <- ""
  if (length(short_hw_type) == 0) short_hw_type = ""
  ...
  if (length(hw_type) > 1 || length(short_hw_type) > 1 || length(hw_num) > 1 ||
      length(hw_title) > 1) {
    hw_id = unique(homework_entries$homework_id)
    stop("Heterogeneous homework type in make_homework_name: [",
         str_c("(", hw_title, hw_id, ")", sep = ": ", collapse = ", "), "]")
  }
...
make_homework_assignment <- function(homework_entries) {
  hw_type = unique(homework_entries$hw_type) %>% discard(is.na)
  med_hw_type = unique(homework_entries$med_hw_type) %>% discard(is.na)
  if (length(hw_type) == 0) hw_type <- "Homework"
  if (length(med_hw_type) == 0) med_hw_type <- "Homework"

  if (length(hw_type) > 1 || length(med_hw_type) > 1) {
    hw_id = unique(homework_entries$homework_id)
    stop("Heterogeneous homework type in make_homework_assignment: [",
         str_c("(", hw_id, hw_type, ")", sep = ": ", collapse = ", "), "]")
  }

  hw_type <- str_trim(hw_type)
  med_hw_type <- str_trim(med_hw_type)
  ...
  homework_entries <- homework_entries %>%
    ...
    distinct() %>%
    mutate(homework = map_chr(homework, markdown_and_expand) %>%
             str_trim() %>% ifelse(str_length(.) == 0, NA_character_, .),
           homework_notes = map_chr(homework_notes, markdown_and_expand) %>%
             str_trim() %>% ifelse(str_length(.) == 0, NA_character_, .))
  ...
  if (nrow(grad_notes) + nrow(ugrad_notes) + nrow(everyone_notes) > 0) {
    output <- str_c(str_trim(output),
                    '\\subsection{Notes on ',  med_hw_type, '}\n',
                    '', sep = '\n')
...
make_hw_section <- function(cal_entry) {
  ...
  if (nrow(hw_assignment_details) > 1) {
    stop("Error: heterogeneous homework assignment in make_hw_section: [",
         str_c("(", hw_assignment_details$homework_id, ")",
               sep = ": ", collapse = ", "), "]")
  }

  hw_title <- make_homework_name(homework)
...
make_short_hw_assignment <- function(cal_entry) {
  ...
  hw_assignment_details <- homework %>%
    ...
    distinct()
  if (nrow(hw_assignment_details) > 1) {
    stop("Error: heterogeneous homework assignment in make_short_hw_assignment: [",
         str_c("(", hw_assignment_details$homework_id, ")",
               sep = ": ", collapse = ", "), "]")
  }
...
format_class_entry <- function(class_no, is_homework) {
  cal_entry <- calendar %>% filter(class == class_no)
...
format_assignment_entry <- function(class_seq_no, is_homework) {
  cal_entry <- calendar %>% filter(seq == class_seq_no)
  hw_entry <- homework_assignments %>% filter(homework_id %in% cal_entry$homework_id)
  notice_entry <- notices %>% filter(topic_id == cal_entry$topic_id)
